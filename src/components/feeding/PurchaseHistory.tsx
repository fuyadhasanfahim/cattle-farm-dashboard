...
export default function PurchaseHistory() {
    ...
    const [data, setData] = useState<IFeedPurchase[]>([]);
        ...
        const fetchData = async () => {
            try {
                ...
                const response = await fetch(
                    `/api/feeding/purchases/get-purchases?search=${search}&limit=${itemsPerPage}&page=${currentPage}`
                );

                const result = await response.json();

                if (response.ok) {
                    setData(result.feedPurchases || []);
